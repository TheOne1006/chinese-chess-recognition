...
optim_wrapper = dict(
    optimizer=dict(
        ...
        betas=(0.9, 0.999)),
        paramwise_cfg=dict(
            norm_decay_mult=0.05,
            bias_decay_mult=0.05,
            flat_decay_mult=0.05,
            custom_keys={
                '.absolute_pos_embed': dict(decay_mult=0.05),
                '.relative_position_bias_table': dict(decay_mult=0.05)
            }),
)
# learning policy
param_scheduler = [
